...
st.markdown("""
    ...
    /* 파일 업로더(카메라 호출) 버튼 대형화 및 가시성 극대화 */
...
st.caption("순차 가이드형 워크플로우 & 스마트폰 카메라 하드웨어 연동 버전 (V7.0)")
st.write("---")
...
if "step" not in st.session_state:
    st.session_state.step = "MASTER_WAIT"  # 단계 상태: MASTER_WAIT -> TEST_WAIT -> RESULT
...
if st.session_state.step == "MASTER_WAIT":
    ...
    # 폰의 원본 카메라를 호출하는 업로더 (용량 튕김 버그 완벽 대항)
    master_file = st.file_uploader("🎯 여기에 터치하여 [기준 마스터] 촬영", type=["jpg", "jpeg", "png"], key="cam_master_raw")
    ...
    if master_file:
        ...
        st.session_state.step = "TEST_WAIT" # 촬영 즉시 다음 단계로 자동 강제 점프
        st.rerun()
...
# [2단계] 생산 제품 대조 촬영 대기 상태
    ...
    test_file = st.file_uploader("🔍 여기에 터치하여 [생산 제품] 촬영", type=["jpg", "jpeg", "png"], key="cam_test_raw")
    ...
    if test_file:
        ...
        st.session_state.step = "RESULT" # 촬영 즉시 결과 단계로 자동 강제 점프
        st.rerun()
...
# [3단계] 최종 판정 결과 확인 및 다음 액션 유도
    ...
    with act_col1:
        if st.button("🔄 다음 생산제품 추가 검사 (매시간 검사)", key="next_test"):
            st.session_state.test_img = None
            st.session_state.test_txt = ""
            st.session_state.step = "TEST_WAIT" # 기준은 유지하고 검사 카메라 단계로 백
            st.rerun()
            
    with act_col2:
        if st.button("🆕 완전히 새로운 기준 등록 (교대조/제품변경)", key="reset_all"):
            st.session_state.clear() # 모든 정보 포맷 후 1단계로 백
            st.rerun()
